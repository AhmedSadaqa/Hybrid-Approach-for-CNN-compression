...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            neurons_left = int(group_wd_optimizer.get_number_neurons(print_output=False))
            flops = int(group_wd_optimizer.get_number_flops(print_output=False))
            print('neurons_optimizer_left', neurons_left)
            print('flops', flops)
...
if 1:
    group_wd_optimizer = group_lasso_decay(parameters_for_update, group_lasso_weight=1e-8, named_parameters=parameters_for_update_named, output_sizes=output_sizes)
